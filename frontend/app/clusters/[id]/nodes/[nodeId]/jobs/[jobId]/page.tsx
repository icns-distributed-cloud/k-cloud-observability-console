...
export default function JobPage({
    ...
    const { id, nodeId, jobId: jobIdStr } = use(params);
    ...
    useEffect(() => {
        Promise.all([
            ...
            fetchClusterDetail(clusterId).catch(() => null),
            fetchNodeDetail(nodeIdNum).catch(() => null),
        ])
            .then(async ([j, k, r, a, n, c, nd]) => {
                setJob(j);
                setKqv(k);
                setReallocs(r);
                setAdjustments(a);
                setNegotiation(n);
                setClusterName(c?.name ?? `클러스터 ${clusterId}`);
                setNodeName(nd?.name ?? `노드 ${nodeIdNum}`);

                const ml = await fetchModelLayers(j.model_id).catch(() => null);
                setModelLayers(ml);
            })
            .catch((e) => setError(String(e)));
    }, [jobId, clusterId, nodeIdNum]);
            ...
            {tab === "overview" && (
                <div>
                    ...
                    {modelLayers && modelLayers.layers.length > 0 && (
                        <>
                            <div style={{ ...SECTION_LABEL, marginTop: 24 }}>
                                모델 분석 · 레이어 연산 그래프
                            </div>
                            <Card>
