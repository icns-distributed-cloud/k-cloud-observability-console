...
export default function AllocationTimeline({ data, onSelectJob, onSelectNode, showActiveModel, admittedFlashes }: Props) {
  ...
  return (
    <div style={{ display: "flex", gap: 20 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        {/* 열 제목 - 왼쪽 라벨 칸은 노드, 막대 영역은 타임라인.
            아래 relative 영역 밖에 둬야 한다 - 현재시각 선과 flash 위치가 그 영역을
            기준으로 계산돼서, 헤더가 안에 들어가면 전부 헤더 높이만큼 밀린다. */}
        <div
          style={{
            display: "flex",
            alignItems: "center",
          ...
          {showActiveModel && (
            <span
              style={{
                ...
                letterSpacing: "0.06em",
                color: "var(--ink)",
                // 열 사이를 세로선으로 가른다. gap 안쪽이 아니라 각 열의 왼쪽에 붙여야
                // 아래 데이터 행의 열 경계와 정확히 맞는다.
        ...
        <div style={{ position: "relative" }}>
          {data.rows.map((row) => {
                ...
                return (
                  ...
                  >
                    <span style={{ width: LABEL_W, flexShrink: 0, display: "flex", justifyContent: "flex-end" }}>
                      <span
                        title={row.nodeName}
                        onClick={onSelectNode ? () => onSelectNode(row.nodeId) : undefined}
                        className={onSelectNode ? styles.nodeLink : undefined}
                        style={{
                          // 추론 섹션은 오른쪽 막대가 주황이라 노드 박스도 같은 색 계열로 맞춘다
                          "--node-link-color": showActiveModel ? "var(--job-infer)" : "var(--accent)",
                          padding: "4px 10px",
                          borderRadius: 6,
                          fontSize: 15,
                          fontWeight: 700,
